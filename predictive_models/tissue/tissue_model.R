...
#MODASE_cat
#glm_fit_data_modase <- select(wasp_final_noagg, CHR, MODASE_cat, `50_BP_RULE`, Long_Exon, Near_Start, LoF, cDNA_position_only, CDS_position_only, Protein_position_only, GerpN:PHRED, gnomAD_AF, TISSUE_ID) %>% 
    #filter(!is.na(`50_BP_RULE`)) %>%
    #filter(nchar(`50_BP_RULE`) > 0) %>%
    #filter(!is.na(GC))
...
#REF_RATIO_cont
glm_fit_data_refratio_cont <- select(wasp_final_noagg, CHR, logit_REF_RATIO, TOTAL_COUNT, `50_BP_RULE`, Long_Exon, Near_Start, LoF, cDNA_position_only, CDS_position_only, Protein_position_only, GerpN:PHRED, gnomAD_AF, TISSUE_ID, SMRIN, SMTSISCH, SEX, AGE, DTHHRDY) %>% 
    filter(!is.na(`50_BP_RULE`)) %>%
    filter(nchar(`50_BP_RULE`) > 0) %>%
    filter(!is.na(GC)) %>%
    filter(!is.na(SMTSISCH)) %>%
...
#MODASE_cat
#glm_fit_data_modase_cont <- select(wasp_final_noagg, CHR, logit_MODASE, `50_BP_RULE`, Long_Exon, Near_Start, LoF, cDNA_position_only, CDS_position_only, Protein_position_only, GerpN:PHRED, gnomAD_AF, TISSUE_ID) %>% 
    #filter(!is.na(`50_BP_RULE`)) %>%
    #filter(nchar(`50_BP_RULE`) > 0) %>%
    #filter(!is.na(GC))
